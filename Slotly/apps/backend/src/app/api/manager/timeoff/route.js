...
export async function PATCH(request) {
    try{
        ...
        // If your actually reading my code your weird get a life or just do it for me
        // Notifications set up placeholder
        // data:{
        // userId: requestEntry.staff.id,
        // type: 'TIME_OFF_REQUEST',
        // title: `Time Off Request ${status}`,
        // message: `Your time off request from ${requestEntry.startDate.toISOString()} to ${requestEntry.endDate.toISOString()} has been ${status.toLowerCase()}.`,
        // }
        // });
...
export async function PUT(request) {
  try {
    ...
    const requestEntry = await prisma.timeOffRequest.findUnique({
      where: { id },
      include: { staff: { select: { staffOf: true } } }
    });
    ...
    console.error('PUT /manager/timeoff error:', err);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
...
export async function DELETE(request) {
  try {
    ...
    console.error('DELETE /manager/timeoff error:', err);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}